...
//    private List<String> conversations = new ArrayList<String>();
    private TreeMap<String,ConversationAux> conversationsMap = new TreeMap<String,ConversationAux>();
...
//    }
    ...
    public DataModel getConversationsMapDataModel() {
        List l = new ArrayList(this.conversationsMap.values());
...
//    }
    ...
    public void beginConversation(String name) {
        if (this.conversation.isTransient()) {
            ...
            aux.setName(name);
            this.conversationsMap.put(name, aux);
    ...
    public void endALlConversations() {
        List<ConversationAux> values = new ArrayList(this.conversationsMap.values());
    ...
    public void endConversation(String name) {
        ConversationAux aux = this.conversationsMap.get(name);

        ManagedConversation managedConversation = this.conversationContext.getConversation(aux.getId());

        if (!managedConversation.isTransient()) {
            managedConversation.end();
            this.conversationsMap.remove(name);
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage("The Conversation with name " + name + " was killed sucessfully"));
        }
        else {
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage("The Conversation with name " + name + " was can't killed because not is Long Running"));
        }
    }
    
    public boolean conversationContextContainsConversation(String id) {
        System.out.println("id: " + id);
        if(id == null) {
            return false;
        }
        List<ManagedConversation> list = new ArrayList<ManagedConversation>(this.conversationContext.getConversations());
        System.out.println("mc.size: " + list.size());
        
        boolean r = false;

        for (ManagedConversation mc : list) {
            System.out.println("mc.id: " + mc.getId());
